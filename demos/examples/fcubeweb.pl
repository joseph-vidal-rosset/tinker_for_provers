...
h_Decide(X, COUNTERMODEL, NUMERO) :- writeln('Input Formula:'),
    printSWFF(swff(f, X)),
    writeln('\n'),
...
h_Decide(_, [valida], NUMERO) :- writeln(' '),
    write(NUMERO),
...
/* se non ci sono candidati, allora non c'è nulla da semplificare */
...
iteraSemplificazione([A|L], NewSet, NewerSet) :- append([A|L], NewSet, TempSet), !,
    simplification([A|L], TempSet, NewTempSet), !,
...
andTable2(VALX, VALY, (VALX & VALY)). /* altrimenti il valore è l'and del valore dei 2 congiunti */
...
orTable2(VALX, VALY, (VALX | VALY)). /* altrimenti il valore è l'and del valore dei 2 congiunti */
...
imTable2(VALX, VALY, (VALX => VALY)). /* una wff,allora il valore è VALX->VALY */
...
simplification([], X, X).
simplification(_, X, X) :- not(coerente(X)),
    !.
...
riprovaSimplification(_, _, TEMPNEWSET, TEMPNEWSET) :- not(coerente(TEMPNEWSET)).
...
intControModello([swff(fperm, list(Wff, Atoms))], RESTO, MODEL, SPAZIO, WhichBranch, IdxNewAtom) :- !,
    /* nota che adesso rule è a 5 parametri */ rule(swff(fperm, list(Wff, Atoms)), RESTO, RESULT, WhichBranch, IdxNewAtom),
...
intControModello([H], RESTO, MODEL, SPAZIO, WhichBranch, IdxNewAtom) :-
    !,
    /* nota che adesso rule è a 5 parametri */ rule(H, RESTO, RESULT, WhichBranch, IdxNewAtom),
...
buildBack(SET, [], SET). /* attenzione: questo caso unfica anche con tutti gli altri precedenti,non è in esclusione */
...
clSimplification(Set, NewSet) :- buildFalseAtomicSwffs(Set, SetOfFalseAtomicSwffs),
    ...
    simplification(SetOfFalseAtomicSwffs, SimpSet, NewSet).
